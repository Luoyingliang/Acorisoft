...
class Program
{
    ...
    {
        httpPostRequest.Prefixes.Add("http://localhost:8008/");
            ...
            Thread threadsub = new Thread(new ParameterizedThreadStart((requestcontext) =>
            {
                    ...
                    if (key.type == 1)
                    {
                        ...
                        if (!string.IsNullOrEmpty(fileName))
                        {
                            string filePath = AppDomain.CurrentDomain.BaseDirectory +
                                              DateTime.Now.ToString("yyMMdd_HHmmss_ffff") +
                ...
                byte[] buffer =
                    System.Text.Encoding.UTF8.GetBytes(
                        Newtonsoft.Json.JsonConvert.SerializeObject(new {success = "true", msg = "提交成功"}));
